...
export function shouldTreatSystemAsClosed(stations: Station[]): boolean {
    const installedStations = stations.filter((station) => station.is_installed);
    const closedStations = installedStations.filter(
        (station) => !station.is_renting && !station.is_returning
    ).length;
    const closedRatio = installedStations.length === 0 ? 1 : closedStations / installedStations.length;

    const totalRentableBikesAvailable = installedStations.reduce((sum, station) => {
        if (!station.is_renting) return sum;
        return sum + Math.max(station.num_bikes_available, 0);
    }, 0);

    return closedRatio >= CLOSED_RATIO_THRESHOLD || totalRentableBikesAvailable === 0;
}
...
async function fetchStations({allowClosed = false}: {allowClosed?: boolean} = {}): Promise<Station[]> {
    ...
    if (!allowClosed && shouldTreatSystemAsClosed(stations)) {
        throw new SeasonClosedError();
    }
    return stations;
}
