import {pickNearestPickupStation, pickNearestRentalStation, pickNearestStation} from "./stations";
...
describe("pickNearestPickupStation", () => {
    it("accepts stations that can rent bikes even if returning is temporarily disabled", () => {
        const stations: Station[] = [
            makeStation({
                station_id: "renting-only-nearby",
                lat: 43.0732,
                lon: -89.4012,
                is_returning: false,
            ...
            makeStation({
                station_id: "full-service-farther",
                lat: 43.08,
                lon: -89.41,
                num_bikes_available: 3,
            }),
        ...
        const nearest = pickNearestPickupStation(stations, {lat: 43.0731, lon: -89.4012});

        expect(nearest?.station_id).toBe("renting-only-nearby");
    });
});

describe("pickNearestRentalStation", () => {
    it("uses renting-only stations as the nearest fallback even when they cannot accept returns", () => {
        const stations: Station[] = [
            makeStation({
                station_id: "renting-only-nearby",
                lat: 43.0732,
        ...
        const nearest = pickNearestRentalStation(stations, {lat: 43.0731, lon: -89.4012});
